...
ACCEPT input_tbs CHAR prompt 'Name of existing encrypted tablespace: '

spool &input_tbs..validate-after.log replace
...
if tbs_is_enc = 0 then
	dbms_output.put_line('Tablespace '''||tbs_name||''' is NOT encrypted!');
else		-- tbs_is_enc = 0
...
-- Start Index Loop:
	...
	if dbms_lob.instr(index_ddl, 'INDEXTYPE IS "CTXSYS"') > 0 then
		...
		dbms_output.put_line(CHR(13));
		dbms_output.put_line('-- Table with Spatial index '||owner||'.'||iname(j)||':');
	end if;
